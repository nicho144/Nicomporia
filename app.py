...
# Fetch Fed Funds Futures and calculations (Real and Implied)
def get_fed_funds_data():
    effr = get_fred_data("EFFR")  # Federal funds rate
    # Assuming implied rate is based on futures pricing
    fed_funds_futures = yf.download("FF=F", period="1d", interval="1m")  # Example ticker for Fed Funds Futures
    if effr is not None and fed_funds_futures is not None:
        real_interest_rate = effr.tail(1).iloc[0]
        implied_rate = fed_funds_futures['Adj Close'].iloc[0]  # Example for implied rate
        return real_interest_rate, implied_rate, fed_funds_futures
...
def get_fed_funds_calculations():
    real_interest_rate, implied_rate, _ = get_fed_funds_data()
