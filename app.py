...
# Plotting function for term structure (with error handling)
def plot_term_structure(symbol):
    ...
    if prices:
        fig, ax = plt.subplots()
        ax.plot(valid_contracts, prices, marker='o')
        ax.set_title(f"{symbol} Term Structure")
        ax.set_xlabel("Contract")
        ax.set_ylabel("Price")
        st.pyplot(fig)
    else:
        st.warning(f"No valid data available for {symbol} term structure.")

# Gold contango interpretation (safe version)
def gold_contango_status():
    try:
        front_data = yf.Ticker('GC=F').history(period='1d')
        back_data = yf.Ticker('GCQ24.CMX').history(period='1d')

        if front_data.empty or back_data.empty:
            return "Gold data unavailable for term structure"

        front = front_data['Close'].iloc[-1]
        back = back_data['Close'].iloc[-1]
        ...
        return f"Error fetching gold term data: {e}"
