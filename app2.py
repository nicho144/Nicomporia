...
Builds a live financial dashboard showing yield curve, VIX term structure,
risk-on/off signals, and institutional money flow prediction.
"""
...
def init_fred():
    try:
        fred = Fred(api_key=st.secrets["dceb19d6ccadfb4115c3ac2493b7d5ed"])
        return fred
    except Exception as e:
        st.error("Missing or invalid FRED API key. Please check secrets.toml")
        st.stop()
...
st.sidebar.info("Tracks Treasury Yields, VIX Term Structure, "
                "Market Futures, and predicts Institutional Money Flow.")
